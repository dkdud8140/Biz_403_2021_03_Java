 ...
 * 
 * class, method, 맴버변수를 선언할 때 사용하는 Keyword
 * 접근제한자는 사용하지 않아도
 * 문법적인 오류가 발생하지는 않는다.
 * 
 ...
 * 			obj.변수 = 100 ;
 * 
 * private : 현재클래스(파일)에 있는 코드에서만 접근가능.
 * 			obj.변수 = 100 ; // 문법오류
 * 
 * protected : 생성된객체를 통해서는 직접 접근금지
 * 			Class obj = new Class();
 * 			obj.method() 방식으로는 접근이 안된.
 * 			클래스를 상속받게 하겠다는라는 전제하에 상속을 받은 
 *   		클래스에서는 자유롭게 접근가능
 * 			
 * 			부모클래스와 상속받은 자식 클래스간에 공유하는 객체
 */
public class ScoreServiceV1 {
	/*
	 * 맴버변수를 protected로 선언을 해두면 현재클래스(V1)를 상속받은 자식 클래스에서 변수가 선언된 것처럼 동작할 수 있다. 상속을
	 * 허락하고, 맴버변수를 공유하고자 할 떄는 protected를 추가한다.
	 */
	...
	public Integer inputScore() {
		...
		while (true) {
			...
			if (intKor == -1) {
				...
				System.out.println("국어점수는 0 ~ 100까지만 입력하세요");
				System.out.println();
				continue;
			}
			break;
		...
		while (true) {
			...
			if (intEng == -1) {
				...
				System.out.println("영어점수는 0 ~ 100까지만 입력하세요");
				System.out.println();
				continue;
			}
			break;
		...
		while (true) {
			...
			if (intMath == -1) {
				...
				System.out.println("수학점수는 0 ~ 100까지만 입력하세요");
				System.out.println();
				continue;
			}
			break;
