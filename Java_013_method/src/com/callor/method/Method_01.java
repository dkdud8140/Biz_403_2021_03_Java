...
public class Method_01 {
	...
	public static void main(String[] args) {
		 ...
		 * Random() 생성장 method를 호출하여 객체를 생성(초기화)하기
		...
		/*
		 * int 형 변수와 
		 * Integer wrapper를 사용하여 만든 변수는 사용하는 기본 방법은 동일.
		 * Integer는 클래스 적인 ㅌ특성을 갖고 있다보니
		 * new Integer() 생성자를 사용하여 변수를 만들면 다소 엉뚱하게 작동되는 경우가 있다.
		 * 지금은 Intefer()생성자를 사용하여 변수를 만들지 못하도록 하고 있다.
		 * 
		 * 숫자형 변수는 생성자를 사용하여 변수를 반들지 말자~!
		 */
		
		// Integer int1 = new Integer(); 		new Integer() 사용금지
		 ...
		 * 조건에 완전히 일치하는 경우에는
		 * ~~하다. ~~다. 라고 표현한다.
		 * 조건에 일치하지 않는 else 를 사용하여 표현할 때는
		...
		if(int1 % 3 == 0) {
			...
			System.out.println(int1 + "은 3의 배수 아님");
		}
		 ...
		 * 코드를 작성할 때 조건으로 명시한 이외의 경우는 가급적
		 * ~~이 아니다 라고 표현하자. 
		 */
		...
		if(int1 % 2 == 0) {
			...
			System.out.println(int1 + "은 짝수가 아님");
		}
		...
		System.out.println(nsV1.inputNum());
		System.out.println(nsV1.inputNum());
		System.out.println(nsV1.inputNum());
		System.out.println(nsV1.inputNum());
