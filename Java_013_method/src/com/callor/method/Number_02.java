...
public class Number_02 {
	public static void main(String[] args) {
		...
		float floNum1 = Float.parseFloat(strNum2);		
		 ...
		 * 데이터가 컴퓨터와 컴퓨터 간에 이동할 때
		 * 데이터를 효율덕이고, 오류가 나지 않도록 이동하는 다양한 방법이 있다.
		 * 여러가지 방법이 취하는 많은 공통된 부분에 
		 * 모든 데이터를 문자열화 하여 전송하는 방법이 있디.
		 * 숫자, 문자, 특수기호 등을 모두 문자열(ASCII코드)로
		 * 전송하고 받는 곳에서 사용하기 전에 적절한
		 * 타입으로 변경하여 사용한다
		 * 
		 * JAVA 에서는 WrapperClass.parse*() 메소드나
		 * WrapperClass.ValueOf() 메소드를 사용하여 문자열형 숫자를 숫자 값으로 변환시키는 것을 지원한다.
		 * 
		...
		intNum1 = Integer.valueOf(" " + 30) ;
		intNum1 = Integer.valueOf(" 100");	//40번 코드와 41번 코드는 같다.
		intNum1 = Integer.valueOf("100 ");
